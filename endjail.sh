...
# Creation Date: January 07, 2017
# Version: 0.01
# Revision Date: January 07, 2017
# 
# Description: Applies a network permisive based firejail policy to any invoked program
#
...
####################################################################
# $ mkdir ~/bin
# $ cp endjail.sh ~/bin/endjail
# $ export PATH=$PATH:~/bin/
# $ cd ~/bin
# $ chmod u+wrx endjail
#  RUN ENDJAIL
# $ endjail links -g 
# $ endjail --no-net xpdf strange.pdf
#####################################################################
#############################################################################################################################################################################
#                                         ACKNOWLEDGMENTS
#############################################################################################################################################################################
...
do
 ...
 if [ "$arg" == "--help" ]
 then
 echo "ENDJAIL:  firejail defaults from the command line"
 echo " "
 echo " USAGE: netjail --option program_name "
 echo " endjail --help    # print usage information"
 echo " endjail --version # print version information"
 echo " endjail --no-net program_name # restrict internet access"
 shift
 exit 0
 elif [ "$arg" == "--version" ]
 then
 echo "ENDJAIL: version "$version", branch: "$branch", revision date: "$rev_date" "
...
if [ "$net" == "on" ]
then
firejail --noprofile --caps.drop=all --netfilter --nonewprivs --noroot --seccomp --protocol=unix,inet,inet6 "$@"
else
firejail --noprofile --caps.drop=all --netfilter --nonewprivs --noroot --seccomp --net=none --protocol=unix "$@"
fi
