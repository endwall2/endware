...
###############################################################################################################################################################
# Program: endwall_raspi.sh
# Type: Bourne shell script
# Creation Date:         Jan 1  2013
# Branch: 2, wifi enabled
# Current Version: 1.43
# Revision Date: July 7, 2022
# Previous Version:  1.42, October 26 2021
# Author: THE ENDWARE DEVELOPMENT TEAM
# Copyright: THE ENDWARE DEVELOPMENT TEAM, 2016
#
# Changes:     - changes to ip grabing to work on raspbian bullseye for raspbery pi 4 model B
#	       - Update EULA, fix typo in if statement to loop over interfaces, add client_out_internal() function 
#              - Added --version flag + update acknowledgements
#              - Switched to bash + added wifi client out functions (internal ip independent)
#
...
int_if2=$(ip link | grep -a "state " | awk -F: '{ if (FNR==3) print $2}'| awk ' {print $1}')

# Grab Gateway Information
gateway_ip=$(ip route | awk '/via/ {print $3}'| head -n 1)
...
host_ip1=$(ip addr | grep -a "scope global"| grep "dynamic"| grep "$int_if1" |awk 'BEGIN  {FS="/"} {if (FNR==1) print $1}'| awk '{print $2}')
host_ip2=$(ip addr | grep -a "scope global"| grep "dynamic"| grep "$int_if2" |awk 'BEGIN  {FS="/"} {if (FNR==1) print $1}'| awk '{print $2}')
...
proto=$1
ports=$2
iptables -A OUTPUT -o "$int_if" -p "$proto" -m multiport --dports "$ports" -m state --state NEW,ESTABLISHED -j PASS
iptables -A INPUT  -i "$int_if" -p "$proto" -m multiport --sports "$ports" -m state --state ESTABLISHED -j PASS
iptables -A OUTPUT -o "$int_if" -p "$proto" -m multiport --sports "$ports" -m state --state NEW,ESTABLISHED -j PASS
iptables -A INPUT  -i "$int_if" -p "$proto" -m multiport --dports "$ports" -m state --state ESTABLISHED -j PASS
}
...
limrate=$3
limburst=$4
iptables -A OUTPUT -o "$int_if" -p "$proto" -m multiport --dports "$ports" -m limit --limit "$limrate"/s --limit-burst "$limburst" -m state --state NEW,ESTABLISHED -j PASS
iptables -A INPUT  -i "$int_if" -p "$proto" -m multiport --sports "$ports" -m state --state ESTABLISHED -j PASS
iptables -A OUTPUT -o "$int_if" -p "$proto" -m multiport --sports "$ports" -m limit --limit "$limrate"/s --limit-burst "$limburst" -m state --state NEW,ESTABLISHED -j PASS
iptables -A INPUT  -i "$int_if" -p "$proto" -m multiport --dports "$ports" -m state --state ESTABLISHED -j PASS
}
...
proto=$1
ports=$2
iptables -A OUTPUT -o "$int_if" -p "$proto" -m multiport --dports "$ports" -m state --state NEW,ESTABLISHED,RELATED -j PASS
iptables -A INPUT  -i "$int_if" -p "$proto" -m multiport --sports "$ports" -m state --state ESTABLISHED,RELATED -j PASS
iptables -A OUTPUT -o "$int_if" -p "$proto" -m multiport --sports "$ports" -m state --state NEW,ESTABLISHED,RELATED -j PASS
iptables -A INPUT  -i "$int_if" -p "$proto" -m multiport --dports "$ports" -m state --state ESTABLISHED,RELATED -j PASS
}
...
client6_out_wifi udp 53,953
client6_out_wifi tcp 53,953
############################################     DHCPv6  Client         ##############################################################################
client6_out_wifi udp 546,547
client6_out_wifi tcp 546,547
#############################################   HTTP HTTPS Client       ##############################################################################
client_out_wifi tcp 80,443
client6_out_wifi tcp 80,443
##########################################    HKP OPEN PGP CLIENT       ##############################################################################
...
# 50000-55663
client_out_rel_wifi tcp 50000:55663
client_out_rel_wifi udp 50000:55663
...
#############################################       SMTP  Client        ##############################################################################
client_out_lim_wifi tcp 25,465,587,2525 5 10
client6_out_lim_wifi tcp 25,465,587,2525 5 10
##########################################         POP3  Client         ##############################################################################
client_out_wifi tcp 110,995
##########################################         IMAP  Client         ##############################################################################
client_out_wifi tcp 143,993
...
#######################################            BOOTP  Client       #######################################################################################
iptables -A OUTPUT -o "$int_if" -d "$gateway_ip" -p udp -m multiport --dports 67,68 -m state --state NEW,ESTABLISHED -j PASS
iptables -A INPUT  -i "$int_if" -s "$gateway_ip" -p udp -m multiport --sports 67,68 -m state --state ESTABLISHED -j PASS
###########################################        ICMP Ping         ###############################################################################
iptables -A OUTPUT -o "$int_if" -p icmp --icmp-type ping  -m state --state NEW,ESTABLISHED -j PASS
iptables -A INPUT  -i "$int_if" -p icmp --icmp-type ping  -m state --state ESTABLISHED -j PASS

# echo reply from ping
iptables -A INPUT  -i "$int_if" -p icmp --icmp-type 0  -m state --state ESTABLISHED -j PASS
# rejection messages
iptables -A INPUT  -i "$int_if" -p icmp --icmp-type 3  -m state --state ESTABLISHED -j PASS
# time out signal
iptables -A INPUT  -i "$int_if" -p icmp --icmp-type 11 -m state --state ESTABLISHED -j PASS
# echo request from ping
iptables -A INPUT  -i "$int_if"  -p icmp --icmp-type 8  -m limit --limit 1/second -m state --state ESTABLISHED -j PASS
...
################################################# MUMBLE CLIENT ###############################################################
client_out tcp 64731
client_out udp 64731
