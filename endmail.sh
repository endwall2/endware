...
do 
  if [ "$arg" == "--help" ]
  then 
  echo "USAGE: $ endmail recipient@hidden.onion \"Re: The Subject\" message.txt "
  echo "USAGE: $ endmail recipient@hidden.onion \"Re: The Subject\" \"Hey how are you? bye!\" "
  echo "USAGE: $ endmail recipient@hidden.onion \" - \" encrypted.asc "
  exit 0
  fi
done
...
torsocks -i -a "$torsocks_ip" -P "$torsocks_port" swaks --server "$rserver" --to "$recipient" --from "$sender" --helo "$hstring" --tls --h-Subject "$subject" --body "$message"
