...
#
############################################################################
#  DEPENDENCIES:  geoiplookup, iplookup.py
####################################################################
#                      INSTRUCTIONS:   
####################################################################
# $ mkdir ~/bin
# $ cp ipsweep.sh ~/bin/ipsweep
...
# $ chmod u+x check16
#  RUN ipsweep
# $ ipsweep 56.145.12.151
# $ ipsweep -e 156.14.58.198
# $ ipsweep --cidr /8 156.14.58.196
#####################################################################
#############################################################################################################################################################################
#                                         ACKNOWLEDGEMENTS
#############################################################################################################################################################################
...
if [ "$1" = "" ]
then
   echo "IPSWEEP: print out the geoip information for an entire CIDR /16 /8 or /24 range"
   echo ""
   echo "USAGE:"
   echo "$ ipsweep --help         # usage messages"
   echo "$ ipsweep --version      # print version information"
   echo "$ ipsweep --cidr /8 54.87.56.105      # check a /8 /16 or /24 around an ip" 
   echo "$ ipsweep 54.87.56.105  # check a /16 around the ip (default) "
   echo "$ ipsweep -e 54.87.56.105  # check a /16 around the ip using endware iplookup "
   echo "$ ipsweep -e --cidr /24 54.87.56.105  # check a /24 subnet around the ip using endware iplookup "
   shift
   exit 0
fi
...
do
 ...
 then
   echo "checkip: version: "$version", branch: "$branch" , revision date: "$rev_date" " 
   echo "Copyright: The Endware Development Team, 2016-2018"
   shift
   exit 0
 elif [ "$arg" = "--cidr" ]
 then
...
#### REMIND THE USER OF THE INPUT IP ADDRESS 
if [ "$lookup_tool" = "geoiplookup" ]
then
echo "$ip"
fi
