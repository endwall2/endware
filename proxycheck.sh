...
########### torsocks proxy IPv4 address and port 
torsocks_ip=127.0.0.1
torsocks_port=9050
#torsocks_ip=192.168.5.194
#torsocks_port=34917
...
echo "proxycheck http_proxies.txt   # check a list of http_proxies"
echo "proxycheck socks_proxies.txt # check a list of socks5 proxies"
echo "proxycheck socks5_proxies.txt # check a list of socks5 proxies"
echo "proxycheck socks4_proxies.txt # check a list of socks4 proxies"
...
prefix=ssl
mode=proxy
elif [ "$infile" = http_proxies.txt ] ; then 
prefix=http
mode=proxy
elif [ "$infile" = socks_proxies.txt ] ; then 
prefix=socks
mode=socks5
elif [ "$infile" = socks5_proxies.txt ] ; then 
prefix=socks5
mode=socks5
elif [ "$infile" = socks4_proxies.txt ] ; then 
prefix=socks4
mode=socks4
else
echo "USAGE:  $ proxycheck ssl_proxies.txt"
echo "USAGE:  $ proxycheck http_proxies.txt"
echo "USAGE:  $ proxycheck socks_proxies.txt"
echo "USAGE:  $ proxycheck socks4_proxies.txt"
echo "USAGE:  $ proxycheck socks5_proxies.txt"
...
######## TILTLE OUTPUT FILES ##############
holder_1="$prefix"_google.tmp
holder_2="$prefix"_youtube.tmp
outfile_1="$prefix"_proxies_gg_rd.txt
outfile_2="$prefix"_proxies_gg.txt
outfile_3="$prefix"_proxies_yt.txt
...
torsocks -a "$torsocks_ip" -P "$torsocks_port" -i curl -m 90 -A "$UA" -H "$HEAD1" -H "$HEAD2" --"$proto" "$proxy"  https://www.google.com >> "$holder_1"
echo "PROXY: "$proxy"" >> "$holder_1" 
echo "PROXY: "$proxy"" > "$holder_2" 
torsocks -a "$torsocks_ip" -P "$torsocks_port" -i curl -m 90 -A "$UA" -H "$HEAD1" -H "$HEAD2" --"$proto" "$proxy"  https://www.youtube.com >> "$holder_2"
echo "PROXY: "$proxy"" >> "$holder_2" 
echo "$proxy" 
echo " " 
}
