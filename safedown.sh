...
# DESCRIPTION: Safely downloads a file to /dev/shm/temp
...
#
#####################################################################################
# DEPENDENCIES: firejail, tor, torsocks, wget
#####################################################################################
# INSTRUCTIONS: 
#  STEP 1) RETRIEVE PACKAGES 
# $ sudo pacman -S firejail tor torsocks wget 
# $ sudo apt-get install firejail tor torsocks wget
# $ sudo yum install firejail tor torsocks wget 
#
#  STEP 2) MAKE SOME DIRECTORIES
# $ mkdir ~/bin
# $ cp http://ix.io/Nfg ~/bin/safedown
...
echo "$UA"
 if [ "$headmode" == "on" ]
 then 
 # initate wget download +tor + headers + user agent
 firejail --noprofile --protocol=inet --private-tmp --private-etc=resolv.conf --nogroups torsocks -i -a "$torsocks_ip" -P "$torsocks_port" wget --user-agent="$UA" --header="$HEAD1" --header="$HEAD2" --header="$HEAD3" --header="$HEAD4" "$@" 
 else
 firejail --noprofile --protocol=inet --private-tmp --private-etc=resolv.conf --nogroups torsocks -i -a "$torsocks_ip" -P "$torsocks_port" wget --user-agent="$UA" "$@" 
 fi
else 
 firejail --noprofile --protocol=inet --private-tmp --private-etc=resolv.conf --nogroups torsocks -i -a "$torsocks_ip" -P "$torsocks_port" wget "$@" 
fi 
